...
function drawConfiguration() {
  for (var x = 0; x < plot.width; x++) {
    for (var y = 0; y < plot.height; y++) {
      // var s = x*2*Math.PI/plot.width ;
      // var e = y*2*Math.PI/plot.height;
      var s = ((x - plot.width / 2) * 2 * Math.PI) / plot.width;
      var e = ((y - plot.height / 2) * 2 * Math.PI) / plot.height;
  ...
  ctxConfiguration.save();
  ctxConfiguration.beginPath();
  // var x = ( shoulderAngle + Math.PI ) * plot.width / ( Math.PI * 2 );
  // var y = ( elbowAngle + Math.PI ) * plot.height / ( Math.PI * 2 );
  var x = (plot.width * (shoulderAngle + Math.PI)) / (2 * Math.PI);
  var y = (plot.height * (elbowAngle + Math.PI)) / (2 * Math.PI);
...
function drawWorkspace() {
  var cnv, ctx, gridOptions;

  cnv = document.getElementById("workspace");

  gridOptions = {
    minorLines: {
      separationX: canvasWidth / worldWidth / 10,
      separationY: canvasHeight / worldHeight / 10,
      color: "lightblue"
    },
    majorLines: {
      separationX: canvasWidth / worldWidth,
      separationY: canvasHeight / worldHeight,
      color: "lightblue"
    }
  };
...
function drawGridLines(cnv, lineOptions) {
  ...
  iCount = Math.floor(iWidth / lineOptions.separationX);

  for (i = 1; i <= iCount; i++) {
    x = i * lineOptions.separationX;
    ctx.moveTo(x, 0);
    ctx.lineTo(x, iHeight);
    ctx.stroke();
  }

  iCount = Math.floor(iHeight / lineOptions.separationY);

  for (i = 1; i <= iCount; i++) {
    y = i * lineOptions.separationY;
    ctx.moveTo(0, y);
    ctx.lineTo(iWidth, y);
    ctx.stroke();
  }
